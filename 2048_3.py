...
HEIGHT = GRID_SIZE * (CELL_SIZE + MARGIN) + MARGIN + SCORE_HEIGHT
...
class Score:
    ...
    def draw_score(self, surface):
        score_surface = FONT.render(f"Score: {self.score}", True, (119, 110, 101))
        score_rect = score_surface.get_rect(midtop=(WIDTH // 2, MARGIN))
...
def draw_grid():
    ...
    for r in range(GRID_SIZE):
        for c in range(GRID_SIZE):
            ...
            rect = pygame.Rect(c * (CELL_SIZE + MARGIN) + MARGIN, r * (CELL_SIZE + MARGIN) + MARGIN + SCORE_HEIGHT,
                               CELL_SIZE, CELL_SIZE)
